...
class Parser:
    ...
    def Driver(self,url):
        self.driver = webdriver.Firefox(executable_path=r'c:\geckodriver.exe',options=self.option)
        self.driver.get(url)
        time.sleep(5)
...
class Elements():
    ...
    def MoveMenuCity(self):#Переход в меню местоположения
        self.CityMenu=self.driver.find_element(By.CLASS_NAME,'city-select__text')
        self.CityMenu.click()
        time.sleep(3)
        self.CitySelect=self.driver.find_element(By.ID,'select-city')
    def SerchFO(self):    
        self.Districts=self.CitySelect.find_element(By.CLASS_NAME,'districts') # Получили все ФО
    ...
    def SerchCity(self,Region): #Получили Список городов Региона. Результат Словарь Region+[City]
    ...
    def ShopsAvail(self):
        self.driver.find_element(By.CLASS_NAME,'order-avail-wrap__link').click()#Кнопка наличия
        time.sleep(4)
        self.shops= self.driver.find_element(By.CLASS_NAME,'vue-shop-avail__content') #Выбор поля
        CatalogShopNames=self.shops.find_elements(By.CLASS_NAME,'shop-view__title-p') #Получаем список магазинов
        CatalogProductAvails=self.shops.find_elements(By.CLASS_NAME,'shop-choose-list__issue-date-text')#Получаем наличие
        ShopName=[]
        ProductAvail=[]
    ...
    def FindInfo(self):
        ...
        self.title=self.data.find_element(By.CLASS_NAME,'product-card-top__title').text
        try:
            self.price=self.data.find_element(By.CLASS_NAME,'product-buy__price_active').text
            self.price=self.price.replace(' ','')
            self.price=re.search('\d+₽',self.price).group().replace('₽','')
            print(self.title+'\n'+self.price)
        except:
            self.price=self.data.find_element(By.CLASS_NAME,'product-buy__price').text
            self.price=self.price.replace(' ','')
            self.price=re.search('\d+₽',self.price).group().replace('₽','')
            print(self.title+'\n'+self.price)
